...
| Notes/todos store | Plain markdown (todos.md/inbox.md) via a dedicated connector | Human-editable + syncable; precise tools beat LLM hand-editing; capture never invokes the LLM when edited directly. Completing a todo (via the tool or by hand-marking `- [x]`) moves it out of todos.md into a dated `done/<date>.md` on the next todo op, so the active list stays readable |
