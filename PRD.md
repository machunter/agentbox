...
| Todos & notes (`add_todo`, `list_todos`, `complete_todo`, `add_note`, `search_notes`) | ✅ | Local markdown store; always on; validated live. `add_todo` deterministically skips near-duplicates (token-overlap guard in `internal/mcpnotes`) against both open todos AND ones completed within the last ~30 days — so the same item arriving from two sources (email + Slack) won't create a repeat, and a source message re-seen during a multi-day sweep won't be "resurrected" after its todo was completed. Each todo is tagged with the origin it came from (`src:slack:<channel>[/<thread-ts>]`, `src:email:<sender>#<uid>`, `capture`, `manual`), stored in the markdown line's trailing comment; a near-duplicate from a second origin merges into the existing todo and keeps both. The closing sweep uses that tag to look for the reply where the request came from (Slack for Slack, Sent mail for email) instead of always scanning Sent mail; todos predating the tag fall back to Sent mail. For email-sourced todos the sweep checks `email_exists` first: mail the user deleted or filed out of the inbox counts as an answer in itself, closing the todo without a reply hunt. Slack transcripts mark the owner's own messages `(you)` (from `auth.test`), so "did I already reply" is read off the transcript rather than inferred by matching display names. A filing rubric in the agent prompt does the judgment the code can't: skip spam/cold-outreach and below-the-owner's-role admin tasks, prefer a note when unsure (owner operates at CTO level) |
...
| Email — read (`list_new_emails`, `list_recent_emails`, `search_emails`, `read_email`, `email_exists`, `list_mailboxes`) | ✅ | IMAP, read-only; count-bounded, optional date window (`AGENTBOX_EMAIL_SINCE_DAYS` floor / `since_days`). `list_new_emails` is incremental: a persisted per-mailbox UID watermark (UIDVALIDITY-aware) means briefings only process genuinely new mail, no reprocessing/duplicate todos. Mailbox aliases (`Sent`/`Drafts`/`Trash`/`Junk`/`Archive`) resolve to the provider's real folder via RFC 6154 SPECIAL-USE, so the agent can scan sent mail to e.g. close a todo once a reply went out. `email_exists` answers presence from the envelope alone (no body fetch): mail deleted or filed out of the inbox is treated as no longer needing action, closing the todo it produced |
...
| Slack — read (`list_channels`, `read_channel`, `read_thread`, `search_messages`) | 🟡 | Slack Web API, read-only; single configured token (`AGENTBOX_SLACK_TOKEN`), no OAuth — user token (`xoxp-`) for search, bot token (`xoxb-`) for the rest. Resolves user IDs and `<@mentions>` to names and marks the owner's own messages `(you)`; channel lookup by name or ID; history bounded by `AGENTBOX_SLACK_LOOKBACK_DAYS` / per-call `since_days`. Listing uses `users.conversations` (the token identity's own channels), not the workspace-wide `conversations.list`: membership is what the briefing means by "my channels", and it keeps a large org from rate-limiting a routine run. `429`s are absorbed by the connector (bounded `Retry-After` waits) rather than handed to the model, which otherwise improvises `run_bash sleep` and burns tool-call rounds. Unit-tested; `slack-check` diagnostic. Posting deferred (confirmation-gated, like email send) |
